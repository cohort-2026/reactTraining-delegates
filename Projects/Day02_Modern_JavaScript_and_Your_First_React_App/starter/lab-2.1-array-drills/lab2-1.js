...
const underFive = productsInStock.map(p => p.name)

console.log(`In stock under R500 (3): ${underFive.length}\n${underFive.join(" (in stock) \n")} (in stock)`)
// TODO (step 4): use find to get the product with id 4, and log its name.
const productFour = products.find(p => p.id === 4 )

console.log(`Product 4: ${productFour.name}`)
// TODO (step 5): use reduce to total the price of all in-stock items.
// Remember the starting value.
const reducedPrice = products.filter(p => p.inStock === true)

const totalReducedPrice = reducedPrice.reduce((total, product) => total + product.price, 0)

console.log(`Total price of in-stock items: R${totalReducedPrice}`)
...
const filteredProducts = products.filter(p => p !== 5 )

console.log(`After removing product 5: ${filteredProducts.length} products`)
...
console.log(`Original products: ${newProducts} products, unchanged`)
console.log(`${newProducts}.join("\n")`)