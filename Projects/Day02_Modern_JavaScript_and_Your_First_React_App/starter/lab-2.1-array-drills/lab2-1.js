...
console.log(`Names: ${newArray.join(", ")}`);
...
console.log(`In stock and less than R500 (${underFive.length}):\n${underFive.join(" (in stock)\n")} (in stock)`);
...
const inStockItemsAPrice = products.filter(p => p.inStock);
const allAvailablePrices = inStockItemsAPrice.map(p => p.price);

total = allAvailablePrices.reduce((total, allAvailablePrices) => total + allAvailablePrices, 0);
console.log(`Total price of in-stock items: R${total}`);
...
console.log("After removing product 5 (5 products:");
const stepSevenProducts = products.filter(p  => {
  if(p.id === 5) {
    return;
  }
  console.log(`${p.name}: R${p.price} ${p.inStock? "(in stock)": "(out of stock)"}`);
  return p;
})
...
console.log("Unimmuted products:")
products.map(p => {
  console.log(`#${p.id} ${p.name}: R${p.price} ${p.inStock? "(in stock)": "(out of stock)"}`);
});