...
async function getTodo(id) {
  try {
    ...
    // BUG 2 FIX: Check whether the request was successful
    if (!res.ok) {
      throw new Error(`Request failed: ${res.status}`);
    }
...
async function getProgress(limit) {
  ...
  // BUG 3 FIX: Check whether the request was successful
  if (!res.ok) {
    throw new Error(`Request failed: ${res.status}`);
  }

  // BUG 4 FIX: await res.json()
  const todos = await res.json();
